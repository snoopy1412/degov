...
export const FileUploader = ({ onUpload }: FileUploaderProps) => {
  ...
  return (
    ...
    >
      <input {...getInputProps()} />
      {isDragActive ? (
        <p className="text-[18px] font-semibold text-foreground">Drop your ABI file here</p>
      ) : (
        <>
          <p className="text-[18px] font-normal text-foreground">Drag and drop your ABI file</p>
          <p className="text-[14px] text-foreground">Or click to browse your files</p>
        </>
      )}
    </div>
  );
