    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-[220px] gap-[20px] rounded-[14px] border-none p-[20px] shadow-2xl">
        <div className="flex flex-col gap-[20px]">
          {items?.map((item) => {
            return (
              <div className="flex items-center justify-between gap-[10px]" key={item.title}>
                <div className="flex items-center gap-[10px]">
                  <span className={`size-[10px] rounded-full ${item.color}`}></span>
                  <span className="text-[14px] text-white/50">{item.title}</span>
                </div>
